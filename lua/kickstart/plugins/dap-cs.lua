...
local find_startup_projects = function(cwd, allow_multiple)
  ...
  for _, project_path in ipairs(project_files) do
    ...
    local startup_config_file = vim.fs.find(string.format('%s.runtimeconfig.json', project_name), { path = bin_path, type = 'file' })

    if next(startup_config_file) then
      local dlls = vim.fs.find(string.format('%s.dll', project_name), { path = bin_path, limit = math.huge, type = 'file' })
